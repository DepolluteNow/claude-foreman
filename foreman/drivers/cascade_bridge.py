...
class CascadeBridge(AIBridge):
    ...
    def _detect_method(self) -> str:
        ...
        if ide != "Windsurf":
            raise AIBridgeError(f"Windsurf not running (detected: {ide})")

        # Try IPC first (VS Code Extension Host socket)
    ...
    def _find_ipc_socket(self) -> Optional[str]:
        # VS Code forks expose Extension Host at /tmp/vscode-ipc-*.sock
        import glob
        sockets = glob.glob("/tmp/vscode-ipc-*.sock") + glob.glob("/tmp/windsurf-ipc-*.sock")
        # For now, we don't have an IPC client — always fall back to AppleScript
        return None
    ...
    def recalibrate(self) -> None:
        result = subprocess.run(
            ["osascript", str(APPLESCRIPT_DIR / "windsurf_cascade.scpt"), "recalibrate"],
            capture_output=True,
            text=True,
        )
    ...
    def _applescript_send(self, prompt: str) -> None:
        result = subprocess.run(
            ["osascript", str(APPLESCRIPT_DIR / "windsurf_cascade.scpt"), "send", prompt],
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
    ...
    def _applescript_read(self) -> str:
        result = subprocess.run(
            ["osascript", str(APPLESCRIPT_DIR / "windsurf_cascade.scpt"), "read"],
            capture_output=True,
            text=True,
        )
        return result.stdout.strip()
